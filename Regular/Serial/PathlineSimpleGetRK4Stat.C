...
int main(int argc, char** argv) {
  ...
  // !!! Get RK4 Stat
  std::list<RKInfo> rkInfoList;

  osuflow->SetIntegrationOrder(FOURTH); // set to fourth for verification

  osuflow->GenPathLines(list , FORWARD, 50, 0.0, &rkInfoList);
  ...
  rkInfoIter = rkInfoList.begin();

#if 0
  for (; pIter!=list.end(); pIter++, ++rkInfoIter) {
    ...
    printf("\n");
    RKInfo &info = *rkInfoIter;
    printf("RKInfo: i=%d, Ki = %f %f %f, ref position = %f %f %f %f\n", info.i, info.Ki[0], info.Ki[1], info.Ki[2], info.ref[0], info.ref[1], info.ref[2], info.ref[3]);
  }
#endif
